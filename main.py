...
def paleisti_programa():
    ...
    print("1 – Žodžių sąrašas su eilutėmis")
    print("2 – Žodžių paieška (tinklelis + paveikslėliai + 3 linijų forma)")
    print("3 – Lentelė su linksniais")
    print("4 – Kryžiažodis (paveikslėliai kaip užuominos)")
    print("5 – Parašyk sakinį pagal pavyzdį (paveikslėlis + sakinys, toliau tik paveikslėliai ir linijos)")
    print("6 – Gyvūnai ir jų gyvenamosios vietos (sujunk + parašyk)")
    pasirinkimas = input("Pasirink užduotį (1/2/3/4/5/6): ").strip()

    zodziai = ivesti_zodzius()

    if pasirinkimas == "1":
        generuoti_zodziu_uzduoti(zodziai)
    elif pasirinkimas == "2":
        generuoti_pdf_tinkleli_lentele(zodziai, dydis=15)
    elif pasirinkimas == "3":
        generuoti_linksniu_pdf(zodziai)
    elif pasirinkimas == "4":
        kryziazodis_pdf(zodziai, show_answers=False, size=13, failas="out/kryziazodis.pdf")
    # atsakymų versija:
        kryziazodis_pdf(zodziai, show_answers=True, size=13, failas="out/kryziazodis-atsakymai.pdf")
    elif pasirinkimas == "5":
        pavyzdys_sakinys = input("Įvesk pavyzdinį sakinį (pvz. „Kiškis yra pilkos spalvos“): ").strip()
        pavyzdzio_img_zodis = input("Kokio žodžio paveikslėlį naudoti pavyzdyje? (palik tuščią, jei nereikia): ").strip()
        generuoti_sakini_pagal_pavyzdi(
            zodziai,
            pavyzdys_sakinys=pavyzdys_sakinys if pavyzdys_sakinys else " ",
            pavyzdys_paveikslelis=pavyzdzio_img_zodis
        )
    elif pasirinkimas == "6":
        print("Įvesk gyvūnus (pvz.: paukštis, ežys, ožka, arklys, vilkas, šernas, lapė, karvė, voverė, varlė, kiškis, kiaulė)")
        gyvunai = [z.strip() for z in input("Gyvūnai: ").split(",") if z.strip()]
        if not gyvunai:
            gyvunai = ["paukstis","ezys","ozka","arklys","vilkas","sernas","lape","karve","vovere","varle","kiskis","kiaule"]

        print("Įvesk gyvenamąsias vietas (pvz.: medzio dreve, tvenkinys, tvartas, inkilas, miskas)")
        vietos = [z.strip() for z in input("Vietos: ").split(",") if z.strip()]
        if not vietos:
            vietos = ["medzio dreve","tvenkinys","tvartas","inkilas","miskas"]

        generuoti_gyvunai_ir_vietos(gyvunai, vietos)
    else:
